...
import { LogOut, Loader2, Calendar, CheckCircle2, Clock, Wrench, Package } from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const onSubmit = (data: BorrowFormValues) => {
    // lookup equipment from fetched list
    const equipmentItem = equipment?.find((e: Equipment) => e.id === data.equipmentId);

    // use the typed fields availableQuantity / totalQuantity
  ...
  // availableEquipment should use availableQuantity (typed)
  ...
  return (
    <>
                                ...
                                {equipmentLoading ? (
                                  ...
                                  availableEquipment.map((item: Equipment) => (
                                    <SelectItem key={item.id} value={item.id}>
                                      {item.name} ({item.availableQuantity ?? item.totalQuantity ?? 0} available)
                                    </SelectItem>
                                  ))
                                )}
                              </SelectContent>
      ...
      {/* Not-enough-equipment modal */}
      <Dialog open={notEnoughModal.open} onOpenChange={(isOpen) => setNotEnoughModal((s) => ({ ...s, open: isOpen }))}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Not enough equipment available</DialogTitle>
            <DialogDescription>
              You requested <strong>{notEnoughModal.requested}</strong> but only <strong>{notEnoughModal.available}</strong>{" "}
              {notEnoughModal.itemName ? `of ${notEnoughModal.itemName}` : "are available"}.
            </DialogDescription>
          </DialogHeader>

          <div className="mt-4">
            <p className="text-sm text-muted-foreground">
              Please reduce the requested quantity or contact the admin to arrange multiple reservations.
            </p>
          </div>

          <DialogFooter className="mt-6 flex justify-end gap-2">
            <Button variant="outline" onClick={() => setNotEnoughModal({ open: false })}>Close</Button>
            <Button onClick={() => {
              setNotEnoughModal({ open: false });
              (window as any).__setBorrowFormQuantity?.(notEnoughModal.available ?? 1);
            }}>
